...
class mfnet_core(object):
    ...
    def __call__(self, InputImgs):
        ...
        with tf.variable_scope(self._nameScope, reuse=self._reuse):
            ...
            self.firstVar = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self._nameScope)
        ...
        ## get weights from second layer
        self.secondVar = list(set(self.variables)-set(self.firstVar))
        ...
        self.saver2 = tf.train.Saver(var_list=self.secondVar)
        outputs = h65
...
class MF_Detection(object):
    ...
    def __call__(self, InputImgs):
        ...
        self.detectorSaver = tf.train.Saver(var_list=self.detectorVariables, max_to_keep=4, keep_checkpoint_every_n_hours=2)
        self.secondSaver = tf.train.Saver(var_list=self.secondVariables, max_to_keep=4, keep_checkpoint_every_n_hours=2)
